...
At each step $t$:
\begin{enumerate}
  \item Sample $(s_{t+1}, x_{t+1})$ from the Markov chains as in \S\ref{sec:method:transitions} (\ref{eq:nested-draw}) to (\ref{eq:top-only}).
...
\end{equation}
Higher moments (e.g., $\mathbb{E}[T^2]$) can be obtained from the moment-generating function or from $\mathbf{N}^2$ and higher powers. Thus the full distribution and moments are available in closed form in terms of $\mathbf{Q}$, $\mathbf{N}$, and $\boldsymbol{\alpha}$. When the effective top-level process is not exactly Markov (because of the nested chain), the same formulas apply to the \emph{augmented} chain whose states are $(s_t, x_t)$ (or a finite aggregation thereof), with $\mathbf{Q}$ and $\mathbf{R}$ defined on that augmented space; alternatively, one can approximate by averaging over the stationary distribution of the nested chain per goal to get an effective $\mathbf{Q}$ and then use \eqref{eq:absorb-prob} to \eqref{eq:expected-length} as an approximation.
...
\end{equation}
with $\mu_{q_1}$, $\mu_{q_2}$, $\sigma_{q_1}^2$, $\sigma_{q_2}^2$ from \eqref{eq:fp-moments}. \textbf{Using publish as proxy for subscribe:} let $\mu_{\mathsf{pub},q} = \sum_k p_k h_{k,\mathsf{pub}}(q)$, $\sigma_{\mathsf{pub},q}^2 = \sum_k p_k (h_{k,\mathsf{pub}}(q) - \mu_{\mathsf{pub},q})^2$, and $\delta_{\mathsf{pub}} = \mu_{\mathsf{pub},q_2} - \mu_{\mathsf{pub},q_1}$. The sample size when the outcome is ``ever publish'' is $N_{\mathsf{pub}} \geq (z_{\alpha} + z_{\beta})^2 (\sigma_{\mathsf{pub},q_1}^2 + \sigma_{\mathsf{pub},q_2}^2) / \delta_{\mathsf{pub}}^2$. The ratio $N_{\mathsf{sub}}/N_{\mathsf{pub}}$ (with $N_{\mathsf{sub}}$ from \eqref{eq:fp-N} for subscribe) is closed form; if it exceeds $1$, the proxy yields significant results with fewer trajectories. All of \eqref{eq:fp-absorb} to \eqref{eq:fp-N} are closed form once the per-persona matrices $\mathbf{Q}_k$, $\mathbf{N}_k$, $\mathbf{R}_k$ and hitting vectors $\mathbf{h}_k$ are computed for each $k$ and each $q$ of interest.

\paragraph{Possible user (persona) distributions.}
\label{sec:method:persona-distributions}
In this paper we focus on a \textbf{finite set of personas} $\{\mathbf{u}_1, \ldots, \mathbf{u}_K\}$ with weights $p_1, \ldots, p_K$. This choice yields closed-form expressions for all quantities in \S\ref{sec:method:finite-personas} and aligns with how product and UX teams reason (e.g., 5 to 10 segments with labels and weights). \textbf{Pros} of discrete personas: interpretability, exact formulas, and no Monte Carlo over $\mathbf{u}$. \textbf{Cons}: the number of segments must be chosen; fine-grained heterogeneity is approximated by a finite mixture. Alternative distributions that admit closed-form or tractable solutions include the following. For \emph{continuous} bounded traits, \textbf{Beta} marginals (or a product of Betas) support skewness and stay in $[0,1]$. For latent continuous factors, \textbf{Gaussian} (or \textbf{mixture of Gaussians}) with a \textbf{probit} link yields closed-form expectations of single-transition probabilities; full-chain quantities may require low-dimensional integrals or approximation. \textbf{Logistic link + Gaussian} $\mathbf{u}$ has no elementary closed form but can be approximated (e.g., probit approximation). \textbf{Dirichlet or categorical mixtures} reduce to finite sums. In practice, discrete personas offer the best trade-off between realism and tractability for attribution and design; extension to continuous or mixture distributions for population-level inference is discussed as future research in the outlook (\S\ref{sec:outlook}).
...
% -----------------------------------------------------------------------------
\subsection{Condition for publish as a more powerful and valid proxy for subscribe}
\label{sec:method:proxy-condition}
% -----------------------------------------------------------------------------

We ask: under what condition can we use the \emph{publish} rate both as a \emph{positively correlated proxy} for (future) subscriptions and as an outcome on which \emph{power increases faster} than on the subscribe rate? The first requirement ensures that a higher publish rate is indicative of higher subscription potential; the second ensures that testing $q_2 > q_1$ on publish reaches a given power with fewer trajectories than testing on subscribe.

\paragraph{Positive correlation (validity of the proxy).}
Let $\rho$ be the correlation between $Y_{\mathsf{pub}}$ and $Y_{\mathsf{sub}}$ under the population (e.g.\ \eqref{eq:correlation} or \eqref{eq:fp-corr}). For publish to be a valid proxy for subscription in the sense that ``more publish tends to go with more subscribe,'' we need
\begin{equation}
  \rho > 0. \label{eq:proxy-valid}
\end{equation}
When $\rho > 0$, a system that increases the probability of ever publishing also tends to increase the probability of ever subscribing; when $\rho \leq 0$, the proxy is uninformative or misleading.

\paragraph{Faster power growth (efficiency of the proxy test).}
For the one-sided test $H_0: \mu^{(q_1)} \geq \mu^{(q_2)}$ vs $H_1: \mu^{(q_2)} > \mu^{(q_1)}$ at significance level $\alpha$ and power $1-\beta$, the required sample size per system is $N = (z_\alpha + z_\beta)^2 (\sigma_1^2 + \sigma_2^2) / \delta^2$. Power at sample size $N$ increases with the \emph{standardized effect size} $\delta / \sqrt{\sigma_1^2 + \sigma_2^2}$; so power increases \emph{faster} for the outcome that has the larger standardized effect. The ratio of the sample size for subscribe to that for publish is
\begin{equation}
  \frac{N_{\mathsf{sub}}}{N_{\mathsf{pub}}} = \frac{(\sigma_{\mathsf{sub},q_1}^2 + \sigma_{\mathsf{sub},q_2}^2) / \delta_{\mathsf{sub}}^2}{(\sigma_{\mathsf{pub},q_1}^2 + \sigma_{\mathsf{pub},q_2}^2) / \delta_{\mathsf{pub}}^2} = \frac{\delta_{\mathsf{pub}}^2 / (\sigma_{\mathsf{pub},q_1}^2 + \sigma_{\mathsf{pub},q_2}^2)}{\delta_{\mathsf{sub}}^2 / (\sigma_{\mathsf{sub},q_1}^2 + \sigma_{\mathsf{sub},q_2}^2)}.
\end{equation}
So the power of the publish-rate test increases faster than that of the subscribe-rate test (i.e.\ we need fewer trajectories to achieve the same power when using publish) if and only if $N_{\mathsf{sub}} / N_{\mathsf{pub}} > 1$, i.e.\ the standardized effect size (squared) is larger for publish:
\begin{equation}
  \frac{\delta_{\mathsf{pub}}^2}{\sigma_{\mathsf{pub},q_1}^2 + \sigma_{\mathsf{pub},q_2}^2} > \frac{\delta_{\mathsf{sub}}^2}{\sigma_{\mathsf{sub},q_1}^2 + \sigma_{\mathsf{sub},q_2}^2}. \label{eq:proxy-efficient}
\end{equation}

\paragraph{Combined condition.}
\begin{quote}
\emph{For publish to be both a positively correlated proxy for future subscriptions and an outcome on which power increases faster than for subscribe, the model must satisfy: (i) $\rho > 0$ \eqref{eq:proxy-valid}, and (ii) the standardized effect size for publish is greater than for subscribe \eqref{eq:proxy-efficient}. Under the finite-persona model of \S\ref{sec:method:finite-personas}, both $\rho$ and the ratio $N_{\mathsf{sub}}/N_{\mathsf{pub}}$ are computable in closed form from the per-persona hitting probabilities and moments; one can therefore check this condition for any given $q_1$, $q_2$, and user distribution.}
\end{quote}
When (i) and (ii) hold, using the publish-rate test is justified both interpretively (publish is a valid proxy for subscription) and statistically (fewer samples are needed to detect a true improvement).

\paragraph{When is the result non-trivial?}
The condition (i) $\rho > 0$ and (ii) larger standardized effect for publish than for subscribe is \emph{not} automatic. Correlation $\rho$ depends on the topology of the chain and the user distribution: if publish and subscribe are reached along similar paths, $\rho$ tends to be high; if they are largely mutually exclusive, $\rho$ can be low or negative. The ratio $N_{\mathsf{sub}}/N_{\mathsf{pub}}$ depends on the relative variances and effect sizes of the two outcomes under $q_1$ and $q_2$; it can be above or below 1 depending on how strongly tool quality shifts publish vs subscribe. Thus, for a given experiment, one should \emph{compute} $\rho$ and $N_{\mathsf{sub}}/N_{\mathsf{pub}}$ (both are closed form under the finite-persona model) and check whether the proxy is valid and efficient. A useful visualization is to plot the minimal sample size $N$ (for a fixed power and $\alpha$) when using subscribe vs when using publish as the outcome, as a function of how ``publish-heavy'' the system is (e.g., the ratio of outcome weights for publish vs subscribe). Such a plot shows at a glance when using publish as a proxy reduces the required sample size.
...
In practice we do not observe the latent variables of the nested Markov model; we observe messages, tool usage, terminal outcomes, and, from a labelling system, noisy annotations of persona and goals. For the remainder of this paper we assume the following. \textbf{Labelling setting:} We have access to a system that assigns to each trajectory a \emph{user persona} (or distribution over $K$ personas) and to each message a \emph{goal} and \emph{phase} (e.g., start, continue, success, fail). The number of goals $|\mathcal{G}|$ is known (e.g., from the trajectory or from the label vocabulary). Labelling is \emph{mostly accurate} but has a \emph{small, unknown error rate}: a minority of persona or goal-phase labels may be wrong. We are given a sample of $N$ such \emph{labelled trajectories} and wish to estimate transition probabilities with high confidence (low variance), possibly by allowing a small number of \emph{relabellings} to correct the most impactful errors. This subsection defines the setting and an EM-type procedure to estimate transition probabilities and error rates, and to select which labels to refine so as to maximize confidence in the transition estimates.

\paragraph{Observables and labelled data.}
For each trajectory we observe: (1) the sequence of user and AI messages and tool usage; (2) the terminal outcome (published, subscribed, or stopped). In addition we have \emph{labels} from the labelling system: per trajectory, a persona (or distribution) $\widehat{k}(\tau)$ or $\widehat{p}(k \mid \tau)$; per message $m$, a goal index $\widehat{g}_m \in \mathcal{G}$ and a phase $\widehat{\phi}_m \in \{\mathsf{start},\mathsf{continue},\mathsf{success},\mathsf{fail}\}$. We treat the \emph{true} persona and the true goal-phase sequence as latent; the labels are noisy observations with an unknown error mechanism. The aim is to estimate the transition kernels $\pi_g$, $\pi_{\mathrm{top}}$ and, optionally, the label error rates, and to use or improve the labels so that the resulting transition estimates have minimal variance (maximal confidence).

\paragraph{EM for transition probabilities and label errors.}
We can treat true state sequences as latent and observed labels as emitted from a noisy channel, and estimate both the transition probabilities and the error process with an EM-type algorithm \citep{dawid1979maximum}. A \textbf{perfect labelling algorithm is not required}: EM corrects for label noise by re-estimating transitions from the posterior over true states given the noisy labels, effectively reducing variance and bias from mislabelling. Heatmaps of estimated vs.\ ground-truth transition probabilities from a noisy-label EM run are given in the appendix (Figure~\ref{fig:em-heatmaps}). This mirrors the classical Dawid-Skene setup for observer error rates: instead of multiple observers labelling the same item, we have a single labeller (the system) producing a sequence of labels with unknown per-state or per-transition error rates.

\textbf{Latent variables and parameters.} For each trajectory $\tau$, the latent variables are the true persona $k^*$ and the true sequence of goal-phase states $(g^*_m, \phi^*_m)$. The parameters are: (i) the transition probabilities $\mathbf{P}$ (nested $\pi_g$ and top-level $\pi_{\mathrm{top}}$), and (ii) optionally a \emph{confusion model} for the labeller, e.g.\ $\mathbb{P}(\widehat{\phi}_m = \ell \mid \phi^*_m = \ell')$ for phase labels (and similarly for goal or persona). Under a simple confusion model, the probability of observing label $\widehat{\phi}_m$ given true phase $\phi^*_m$ is a fixed matrix (e.g., high diagonal, small off-diagonal); the error rate is then encoded in the off-diagonal mass.

\textbf{E-step.} Given current parameters $(\mathbf{P}, \text{error model})$, compute the posterior over true state sequences given the observed labels and the trajectory structure. For a Markov chain on states $(g, \phi)$, the forward-backward algorithm (or dynamic programming over segments) yields marginal posteriors $\mathbb{P}(\phi^*_m = \ell \mid \text{labels}, \mathbf{P})$ and pairwise posteriors for consecutive states, which in turn give \emph{expected transition counts} under the posterior.

\textbf{M-step.} Update the transition matrix by normalising the expected counts (from the E-step) so that each row sums to one. If estimating the confusion matrix, update it from the cross-tabulation of (posterior true label, observed label). Iterate E- and M-steps until convergence. The MLE of the transition matrix from fully observed sequences is the normalised count matrix; here, the E-step replaces hard counts with soft counts, and the M-step is unchanged \citep{dawid1979maximum}. Because the likelihood is non-convex, initialization matters; spectral or moment-based initializers can improve convergence \citep{zhang2016spectral}.

\paragraph{Variance of transition estimates and the role of relabelling.}
The variance of the maximum-likelihood transition estimates depends on the \emph{effective counts} in each row of the transition count matrix. For a multinomial row $i$ with count $n_i$, the MLE $\widehat{p}_{ij} = n_{ij}/n_i$ has variance proportional to $p_{ij}(1-p_{ij})/n_i$; confidence intervals and non-asymptotic bounds scale with the inverse of these counts \citep{noe2009estimating, huang2024nonasymptotic}. Mislabelled states inject wrong transitions into the count matrix: they bias the estimate and, by spreading counts across incorrect cells, often \emph{increase} variance (e.g., rare true transitions get undercounted). Correcting a small number of highly influential labels can therefore both reduce bias and \emph{lower the variance} of the transition estimates, increasing confidence.

\paragraph{Selecting labels to relabel so as to maximize confidence.}
Suppose we have a budget of $B$ relabellings (e.g., human or expert corrections). We want to choose which $B$ labelled positions (trajectory, message) to relabel so that, after re-estimation, the transition probability estimates have minimal variance (or minimal expected error). State-of-the-art strategies include the following.

\begin{itemize}
  \item \textbf{Uncertainty-based selection.} After running EM, compute for each labelled position the posterior entropy (or probability of being wrong). Relabel the $B$ positions with highest uncertainty. This targets the labels that the model considers least reliable and that therefore contribute most to variance when left as-is.
  \item \textbf{Variance reduction.} For each candidate position, estimate the expected change in the transition count matrix if that label were corrected (e.g., by sampling from the posterior over the true label and computing the resulting count matrix). Approximate the variance of the transition MLE as a function of the count matrix (e.g., multinomial variance or a non-asymptotic bound \citep{huang2024nonasymptotic}). Select the $B$ relabellings that yield the largest decrease in a scalar summary of variance (e.g., trace or max over transition probabilities).
  \item \textbf{Error-bound minimization.} Treat the problem as selective labelling: choose the $B$ positions that minimize an upper bound on the estimation error of $\mathbf{P}$, e.g.\ by greedily reducing the bound that scales with inverse counts or with the spectral gap of the chain.
\end{itemize}

In all cases, after relabelling the chosen positions, we re-run the EM (or a single M-step with the corrected labels) to obtain the final transition estimates. The result is a transition matrix and, if estimated, an error-rate model that are consistent with the corrected data and that enjoy higher confidence (lower variance) than estimates from the raw labels alone. Automated selection of which labels to relabel (e.g., by uncertainty or variance reduction) is future work; the pipeline above supports a human-in-the-loop relabelling budget when $B > 0$.

\paragraph{End-to-end system.}
A pipeline that takes $N$ labelled trajectories and optionally a relabelling budget $B$ is as follows.

\begin{enumerate}
  \item \textbf{Initialize.} Optionally cluster or preprocess labels to get an initial partition into $|\mathcal{G}|$ goals and a starting transition matrix (e.g., from raw counts or from a spectral/moment method \citep{zhang2016spectral}).
  \item \textbf{EM for transitions and errors.} Alternate E-step (posterior over true state sequences given current $\mathbf{P}$ and error model) and M-step (update $\mathbf{P}$ and confusion matrix from expected counts) until convergence. Output $\widehat{\mathbf{P}}$ and, if used, estimated error rates.
  \item \textbf{Select relabellings (if $B > 0$).} Using uncertainty, variance reduction, or error-bound criteria, select the $B$ positions to relabel. Obtain corrected labels (e.g., from an expert or a more accurate model).
  \item \textbf{Re-estimate.} Replace the selected labels with the corrected ones and re-run EM (or one M-step) to get the final $\widehat{\mathbf{P}}$. Optionally quantify confidence via multinomial CIs or non-asymptotic bounds \citep{noe2009estimating, huang2024nonasymptotic}.
\end{enumerate}

The resulting transition matrices can be plugged into the closed-form formulas in \S\ref{sec:method:finite-personas} for outcome probabilities, sample size, and proxy analysis. Validation can be done by held-out likelihood of terminal outcomes (publish, subscribe, stopped) under the estimated model.
